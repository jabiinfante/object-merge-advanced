...
`hardArrayConcat`                | Boolean | `false` | When set to `true`, an array vs. array merge will always result from a concat opetarion from the `input1` parameter with `input2`, no matter which items are contained on those arrays.
